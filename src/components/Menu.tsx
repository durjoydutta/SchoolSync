    ...
    <div className="h-full text-sm font-medium lg:ml-5 md:pl-1 lg:pl-2 flex flex-col justify-around">
      {
        menuItems.map((i) => (
          <div className="flex flex-col justify-between gap-2" key={i.title}>
            ...
            {i.items.map(item => (
              <Link href={item.href} key={item.label} className="w-full flex justify-center md:justify-start py-1.5 gap-3 text-gray-500 group transition-all duration-300 ease-in-out">
